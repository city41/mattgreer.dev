...
export default ({ children }) => <Article {...meta} img={require('./feature.png')}
  intro='The Sega Saturn is notorious for not being able to pull off transparency effects as well as its competitors. Sometimes developers resorted to a "mesh" effect. Why was that compromise needed?'
>{children}</Article>

<wisdom>This article is based on a YouTube video made by Low Score Boy. See the <a href="#and-thanks-to-low-score-boy">thank you section</a> at the bottom of this post.</wisdom>
...
<img src={require('./img/mmx4SaturnTube.png')} alt="Mega Man X4 showing transparent effect in a tube enclosure" />
...
When it comes time to compose all the layers together into an image to display on the screen, priority is used to decide on order. Each VDP2 layer has a priority set, and *each sprite in the VDP1 frame buffer also has a priority.* This allows different sprites to end up behind and in front of different layers.

<img src={require('./img/composingBlowup.png')} alt="Diagram showing how VPD1 and VPD2 get composed together" />
...
<img src={require('./img/guardianHeroesNoOverlapSprites.png')} alt="Guardian Heroes character wearing a half-transparent cape" />
...
<img src={require('./img/guardianHeroesOverlapTransparentSprites.png')} alt="Guardian Heroes character's cape overlapping with another sprite" />
...
<img src={require('./img/mmx4ViaComposite.jpg')} alt="Mega Man X4 running on a modern TV with composite cables" />
