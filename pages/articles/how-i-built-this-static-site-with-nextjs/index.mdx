...
export default ({ children }) => <Article {...meta} img={require('./feature.svg')} socialMediaImg={require('./feature.png')}
                                          intro="If you poke around this site's source, you'll pretty much only find HTML and CSS. But I built it with Next.JS, a React framework. Seems odd? Here's why.">
    {children}</Article>

## Some quick terminology

There are a few terms commonly used with Next to explain how a page's HTML gets rendered
...
If your entire site can be statically built, then you can tell Next to do just that with the command `next build && next export`. After running this command, you will find the site output at `<project root>/.next/server/pages`. You can take this directory and host it on say GitHub pages.
...
Hydration is wasteful and not needed if the page is truly static. Take this page you're on now, it's just a bunch of text. React offers no benefit. You can tell Next to skip all of this by adding this config object to the page:
...
This is prefixed with `unstable` because this config setting [was recently introduced](https://github.com/vercel/next.js/pull/11949). It is experimental at this point and likely to change, I would not recommend it for something mission critical.

With this config in place, the page will only have HTML, CSS ... and any bespoke JavaScript you add yourself.
...
<wisdom>
    It's not very dangerous as this is being done at build time.
</wisdom>
...
Normall, Next's `<Link>` is how you link between pages in your app. Using it for a fully static site is questionable though, as it ends up doing nothing at all. If you do use it, keep in mind you *must* set the `passHref` prop
...
Otherwise the `a` tag will not get the href, making the link dead when you build the site.
...
* The bespoke code does not get minimized or polyfilled. If you look at the source for this page, you can see the theme switcher code exactly as I wrote it, whitespace and all.

* Also, NextJS does not understand this code. During development, it does not get updated with fast refresh, and I also need to account for dev mode in the code itself. This admittedly is a pretty annoying gotcha.
