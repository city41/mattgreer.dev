...
<div className="caption">from <a href="https://xkcd.com/1319/" rel="noreferrer noopener">XKCD</a></div>
...
<div className="caption">Choose one of these to go into demo mode</div>
...
<pitfall>
    It's true the hardware dictates the game's make up quite a bit, but developers still had a lot of leeway in how they coded their game. This tool needs to address a lot of little differences between games to truly work on the entire library. See the [next steps for the tool]("#next-steps-for-the-tool") section below for more info on that.
</pitfall>

### Tile graphics stored in C ROMs

Neo Geo games are made up of many different types of ROM chips. The C ROM chips store the game's graphic tiles. For example here is the title screen of Real Bout Fatal Fury (a personal favorite).

<img src={require('./rbffTitleScreen.png')} alt="Real Bout Fatal Fury's title screen" />

And here are some of the tiles that make up this screen as show in a [tile viewer I created](https://city41.github.io/neospriteviewer). You can make out pieces of the screen in the various tiles if you look closely.
...
<img src={require('./tileCloseup.png')} alt="Real Bout Fatal Fury tiles up close in the tile viewer" />
...
<img src={require('./rbffSameCharacters.png')} alt="Playing Real Bout Fatal Fury with both players using the same character" />

This is just a simple example, Developers got really clever here and pulled off all kinds of neat tricks with palettes.
...
If you want the nitty gritty details, the [Neo Geo dev wiki's page on sprites](https://wiki.neogeodev.org/index.php?title=Sprites) has it all. It's pretty obtuse, as the target audience is someone who's pretty entrenched in Neo Geo development.
...
<img src={require('./rbff_eastSidePark.gif')} alt="Background from Real Bout Fatal Fury for the Neo Geo" />
...
<div className="caption">Taken from the <a href="https://wiki.neogeodev.org/index.php?title=Auto_animation">Neo Geo Dev wiki</a></div>

## Capturing all of this into an animated gif

We've got lots of different concerns going on for this tool: a running emulator, a user interface, manipulating graphics and the finally creating an animated gif. Turns out the browser is a perfect environment for all of this, as we can do everything we need with JavaScript, HTML5 canvas and Web Assembly. Browsers have gotten so cool!
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/ZB8vdhk8mlM" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
<div className="caption" style={{marginTop: 24, maxWidth: 560}}>Music credit: Road to Tutu by Martijn de Boer (NiGiD) (c) copyright 2020 Licensed under a Creative Commons Attribution Noncommercial  (3.0) license. http://dig.ccmixter.org/files/NiGiD/61093 Ft: Javolenus</div>
...
If you read this far, thanks! If you like quirky video game tech posts like this, I wrote a few more you might find interesting:

* [The Sega Saturn and Transparency](http://www.mattgreer.org/articles/sega-saturn-and-transparency)
* [Squeezing the Arduboy for Every Byte](http://www.mattgreer.org/articles/squeezing-the-arduboy-for-every-byte/)
* [Super Mario World Mega Review](http://www.mattgreer.org/super-mario-world/index.html)
