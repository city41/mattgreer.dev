...
<wisdom>
	...
	<pre> 1101001001.011011</pre>. But for simplicity, I'm just going to pretend they
	use base ten.
</wisdom>
...
A simple way to do that is to decide Blue moves at 1 pixel per frame. So the code might be something like this
...
That's fine, `1` pixel per frame means he moves 60 pixels every second. He'd cross the screen in about 5 seconds. Ok what if we want him to move faster? `2` pixels per frame? He'd cross the screen in about 2.5 seconds. What if that is too fast? There isn't much you can do to deal with that. It's either too slow at `1` pixel, or too fast at `2` pixels. You can try fancy things like only moving him 2 out of every 3 frames. But that gets complex quick, and not to mention you're game is essentially no longer 60fps if you do that.

With fixed point, we could say that Blue moves at `1.2` pixels per frame, or just about any fractional value we want. We can decide exactly how fast Blue moves, move him every frame, and it will all just work, nice and simple.
...
You could also define these as `TO_FIXED(a) (a << DECIMAL_BITS)` but I find this a tad easier to read and reason about. Bit shifting is faster, but no worries, the compiler will make these changes for us.
...
Now things get a tad more complex. When multiplying two fixed numbers together, the "decimal point's location" will double. Basically each fixed number has `SCALE` applied to it, so after a multiplication, the result with have `SCALE*2` applied to it, so a division is needed to bring the result back down to a single `SCALE`.
...
Easier to read, and all resolved by the compiler instead of the cpu. I wrote a little script to generate fixedConstants.h
