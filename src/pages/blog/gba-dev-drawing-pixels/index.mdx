...
The GBA has various background modes, and a few of them turn the background into a bitmap surface you can easily draw into. For this post we will stick with mode 3, but modes 4 and 5 also enable bitmap graphics. In mode 3 the entire background converts into a 240x160, 16 bit bitmap. In other words, the entire screen becomes one image that is made up of direct color entries instead of using palettes.
...
This is about as simple as it gets. In mode 3, each pixel on the screen is individually addressable, and each pixel address in vram stores a 16 bit color.

First we switch the GBA to mode 3 by setting the `REG_DISPCNT` (display controller) register.
...
<wisdom>
	...
	convert any color into the GBA's format.
</wisdom>
...
Thankfully, Tonc has done some of that work for us. The libtonc library contains functions like `m3_rect` and `m3_line` that can be used to draw basic shapes to the screen.
...
Bitmap backgrounds are the easiest way to draw pixels. From here on out it's bit trickier, as instead of a bitmap surface we have tiles to work with.
...
const u16 facePalette[16] = {
	0x7c1f, 0x59da, 0x7fff, 0x3626, 0x1c88, 0x0000,
...
With that in mind, let's write code that will change a single pixel of a sprite. We will take our face sprite and change one of its eyes from white to the same dark color of its mouth.
...
What in the heck is going on here? This ridiculous code is precisely changing the tile such that the left eye's palette index is now 4 (the dark color).
...
Our face sprite is using the very first sprite tile in video ram. Believe it our, `tile_mem[4][0]` is the address of that tile, for detailed reasons we'll ignore today. Since video ram needs to be written 2 bytes at a time, we cast this memory address to a `u16*`, as that makes our life easier.

`tile` is currently pointing at the very start of the tile, but our pixel is in a ways. Since tile data is two pixels crammed into a single byte, and we must deal with double bytes, we are working with 4 pixel chunks at a time. The eye pixel we want to change is in the fifth chunk, so we need to jump forward four chunks.
...
Let's again change the eye to black. This is very similar to before, but now the eye is four tiles over. We need to change `setPixel` to be aware that sprites can be multiple tiles wide (and tall).
...
Notice we are multiplying y by 4? That is because this sprite is 4 tiles wide. So this function as it is written will only work correctly on sprites of a certain size. A simple fix is to pass the function how wide the sprite is.
...
In the Mario example, the bushes might have a tile id of 4. So in the map, there will be three 4's.
...
Just like individual tiles are made up of numbers that tell the pixel what color in the palette it is, tile maps are numbers that tell what tile goes onto that part of the background.
...
And here is the real magic that differentiates this from sprite drawing. We are setting up a tiny little tile map in the corner of the screen.

First we get a pointer to our tile map. This is just a location in video ram where we will write our map.

Then we write the numbers 1 through 4 into the map, corresponding to the upper left corner of the screen. That is what the two for loops are doing. When they finish, the tile map for our background will look like this.
...
By setting up 1-4 in the upper corner like this, that means tiles 1 through 4 can be used to draw on. The rest of the tile map is set to zero, and we will leave tile zero alone so that the rest of the screen remains empty.
