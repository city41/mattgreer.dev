...
The second `runUntil` has no inputs, it just sits there and allows the puzzle timer to run out. When it runs out, it verifies that the `clock_empty` flag was set, and also verifies that `clock_seconds_counter` is zero, which it needs to be so that the clock on the screen that the user sees shows `00:00`.
