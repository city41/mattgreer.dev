...
- Multiplayer play through the wireless adapter.
- And many more.
...
<pitfall>
	Actually, come to think of it, the link cable library says the first window is
	ID 0 and the new window is ID 1, so maybe they aren't backwards? I honestly
	don't know.
</pitfall>
...
It's important that both GBA's start the gameplay at the same time. To accomplish this, I have a "sync" phase just before gameplay starts. Here player 1's GBA (with ID of zero) sends a message. Once player 2's GBA gets that message, it sends one back. Player 2 then waits one frame, and then starts. Player 1 will start as soon as it receives player 2's message. Processing a message takes one frame, so that is why player 2 waits that one extra frame. With this, both sides start at the exact same time.

<pitfall>
	Exact same time, within a range of about 16ms (the length of one frame).
	That's because when the players turned on their GBAs is a factor here.
	Sometimes even this tiny little difference can make the music sound a little
	"off", so I altered my game such that in two player mode, only player 1 can
	play music.
</pitfall>
