...
It uses this angle to move forward
...
As soon as we arrive within the radius, the driver will target the next waypoint, and start to gradually turn towards it.

<SimulationCmp level="smooth-turning-1" />

Well... it's kind of working (watch it go past waypoint 3 to see what I mean). This will get better, but for now let's look at how the turning works.

In the first version, it was easy to point the car directly towards the next waypoint. Just figure out what that angle is and set the car to it. But when gradually turning instead, the car can either turn left or right. How do we know which one is correct?
...
<figure>
	...
	/>
	<figcaption>Turning right is better than turning left</figcaption>
</figure>
...
Now that we know which direction to turn, we now need to know how much to turn each frame to accomplish the smooth turning.
...
We can see how much the car needs to turn by determining the angle between the car's direction vector and the vector from the car to the waypoint. This can again be accomplished by using the dot product.

The dot product will only tell us the size of this angle, nothing about which way to turn, but that's fine as we already have that information.
