...
If you remember back in algebra class, plotting `y = 7` would indeed just be the flat horizontal line we saw just above. Just getting seven chips each round is a quick game over for sure.

## Linear scaling

Linear scaling happens when you add to the chips or the multiplier &mdash; but not both &mdash; steadily each round. Pretend you have Wee Joker and you play your single 2 high card once per round, so Wee Joker progressively adds 8 chips each round.
...
And for completeness, here is this situation as a math function
...
Just like the egg equation above, but multiplying `r` by a larger constant, which explains the steeper line. Still linear growth, but a bit faster at least.
...
<BalatroMathJax
	latex={[
		's = (5 + card value + pluto level\\times10)\\times (1 + pluto level\\times1)',
		's = (5 + 2 + 10p)\\times(1 + p)',
		's = (7 + 10p)\\times(1 + p)',
		's = 7 + 7p + 10p + p^2',
		's = p^2 + 17p + 7',
...
The exponent being 2 is what causes the quadratic growth. The `17p` adds in some bonus linear growth too, but really it's the `p squared` that mostly matters here.
...
<BalatroMathJax
	latex={[
		...
		's = (5 + 2 + r\\times8)\\times(1 + round\\times3)',
		's = (8r + 7)\\times(3r + 1)',
		's = 24r^2 + 8r + 21r + 7',
		's = 24r^2 + 29r + 7',
	]}
...
In both cases, it's n squared, but in the second example that is being multiplied by a larger constant, allowing faster growth. The <BalatroMathJax latex="29r" /> is linear growth just like we saw back in the Pluto example.
...
By themselves, these jokers offer no growth at all. It will just take the no growth case above, and notch it up a few levels, from 7 to 21, but that's it
...
<BalatroMathJax
	latex={[
		's = (base chips + card value)\\times(base mult \\times round\\times hologram multiplier)',
		's = (5 + 2)\\times(1 \\times .25r)',
		's = 7\\times.25r',
		's = 1.75r',
	]}
/>
...
And what the heck, let's break the game! This is obviously not realistic at all. And I don't blame you if you skip ahead in the video (about 3:00 is just before the game breaks). But this hand scores so high it ends up at `naneinf`, which is NaN (not a number) multiplied by 10 to the infinite power
...
Out of curiousity, I thought it'd be interesting to see how planet cards grow.

<BalatroPlanetCardGrowthComparisonGraph />

There's something interesting happening with 3 of a Kind through Flush

<BalatroPlanetCardGrowthComparisonGraph justFour />

it turns out Flush scales the worst of the bunch, and Straight does the best. At first I thought this didn't make sense, but the more I think about it, I think it does. Flushes are easy to get in Balatro thanks to Tarot cards and the like, so it makes sense they get a small penalty here.
