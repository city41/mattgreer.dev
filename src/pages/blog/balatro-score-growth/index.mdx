...
The exponent being 2 is what causes the quadratic growth. The <BalatroMathJax latex="17p" /> adds in some bonus linear growth too, but really it's the <BalatroMathJax latex="10p^2" /> that mostly matters here.
...
In both cases, it's n squared, but in the second example that is being multiplied by a larger constant, allowing faster growth. The <BalatroMathJax latex="29r" /> is added linear growth just like we saw back in the Pluto example.
...
A single multiplicitive joker can really help a lot. But it can only take you so far...
