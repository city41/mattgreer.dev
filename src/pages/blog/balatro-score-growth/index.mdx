...
<BalatroMathJax
	latex={[
		's = pluto level\\times ((10 + card value)\\times 1)',
		's = l\\times ((10+2)\\times 1)',
		's = 12l\\times l',
		's = 12l^2',
	]}
/>

The exponent being 2 is what causes the quadratic growth.
...
<BalatroMathJax
	latex={[
		's = (base chips + card value + round\\times wee chips)\\times(base mult + round\\times swash mult)',
...
In both cases, it's n squared, but in the second example that is being multiplied by a larger constant, allowing faster growth. The `29r` is also linear growth added on as a small bonus, but the total growth still remains quadratic.
