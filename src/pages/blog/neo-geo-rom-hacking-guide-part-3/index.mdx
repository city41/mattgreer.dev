...
So it `not`s the input on the next line (that flips all the bits), then writes it off into memory somewhere (`move.b D0, (-$5da4,a5)`), `not`s it again, flipping all the bytes back, `and`s it with whatever is in D2, saves that value off somewhere, moves a couple more bytes around, then reads `$10fd9c` into D0.
