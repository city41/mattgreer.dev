...
You can even take a commercial E-Reader game and run it in the debugger. It will disassemble the binary and provide a very nice debugging experience. This will be very helpful to further figure out more about how E-Reader cards and ERAPI works.
...
The colors are all weird because my ERAPI emulator is super raw, it's drawing the image using the wrong palettes. A lot more work needs to be done.
...
## z80 apps are kinda script-like

I have found the way E-Reader z80 apps work to be pretty interesting. The emulator kind of co-opted the `halt` opcode to mean "draw a frame to the screen". You can load the `a` register without how many frames it should draw, accomplishing a very simple way to wait in your game.
...
What I find interesting is I just whipped up a typical game engine loop on the spot, just to deal out the cards. I didn't need to hook all this into a main game loop like you do with most game development. The entire rest of the game doesn't know or even care that this happens. I found this approach kind of refreshing.
...
Altogether there are over 100 backgrounds, 800 sounds (both sound effects and music) and over 200 Pokémon sprites stored in the E-Reader's ROM. If you're an E-Reader fan, you might have noticed Nintendo's mini games tend to reuse the same sound effects, music and often the same backgrounds. This is why.
...
A single E-Reader dotstrip can store 2,192 bytes of data, which is nearly 3kb. But there is some overhead with the headers, and error correction, making the actual amount of data I've been able to store on a single strip closer to 2kb. This data is compressed though.
...
Honestly I'm a bit surprised how many strips are in Nintendo's cards. Based on the game contained inside and my experience writing Solitaire, it seems like some of their games could have been done in fewer dotstrips. But that's just a total guess, I don't really know.
