...
For this part, we will be changing what the blue dino says while a single player game is playing from "PUSH 2P" to "ROM HAK"
...
<wisdom>
	Why does it say the sprite is at (256,696)? That can't be right, that would be
	off the screen! Strangely, a Neo Geo's sprite coordinate is defined as
	"496-Y". So when it says 696, the Neo Geo will actually put the sprite at
	y=200 on the screen. Yeah, I have no idea why.
</wisdom>
...
<wisdom>
	...
	game Power Spikes 2. The game pulls almost all of its tiles from the third
	pair, and only a few from the first two pairs. I'm guessing reusing Power
	Spike 2's graphics was easier than making new ones. I find this fascinating as
	Power Spikes 2 was made by a different company.
</wisdom>
...
Then do `yarn ts-node src/tools/txtPalToPngPal.ts palette.txt`, then result will be `palette.txt.png`, which looks like this (blown up)
...
The first color in the palette is black, but it's really transparent (the first color in all Neo Geo palettes is always transparent). Due to limitations in a tool we are about to use, we need to change that first color to pure magenta.
...
Can't we just have the hack replace the real "PUSH 2P" speech bubble graphics with the "ROM HAK" graphics? Yes we totally could. We could have had `patchRom` stick one tile at a time into the correct location in the CROM to replace the real graphics with our own. If we did that, the PROM patches would not be necessary. In this guide I did it with the PROM patches as in most real ROM hacks just replacing existing tiles won't always be feasible. Knowing about tilemaps and how to change them is useful info in general, so that's why we did it that way.
...
All Neo Geo games use CROMs and they are always used in the same way. So that part of this hack will apply to any Neo Geo game. But the way games define tilemaps in the PROM will vary from game to game. You might even find a game that doesn't use tilemaps at all but has some other way of loading up the graphic tiles. That is why getting good at figuring out how a game works using MAME's debugger and Lua script is key.
...
Part three will look at how to make logic changes to a Neo Geo game. In other words, how to make ROM hacks that change how the game behaves. That is a more involved topic, so it might take me a while to write it. But the good news is it will use all of the same tools covered in this part.
