...
<wisdom>
	Actually the address is <pre>0x80223f0</pre>. This is because GBA games run in
	Thumb mode. Functions will always be at an even address due to how instruction
	alignment works on ARM processors. So the CPU takes advantage of that unused
	lowest bit to use that as a "this function runs in Thumb mode" flag. So even
	if the processor jumps there in ARM mode, it will know to switch to Thumb
	before executing this function.
	<br />
	<br />I didn't know this. Thanks to the many people who pointed this out.
</wisdom>
...
<pitfall>
	Actually, sometimes these two bytes are zero and it still decompresses. So not
	quite sure what those bytes are, or if they even matter...
</pitfall>
...
At the end of the day, I should have been able to do this without Ghidra, right? This function in hindsight is very obvious. And that is true. What tripped me up were those two starting bytes. When you make a VPK using `nevpk`, the first four bytes are "vpk0". So I figured that is where I needed to set the pointer. It took Ghidra for me to realize the pointer needs to start 2 bytes in front of that.
