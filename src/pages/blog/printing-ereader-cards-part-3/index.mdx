...
> I have printed E-Reader cards using a Kodak digital printing press to great success. They are the best scanning cards I have ever seen (other than the Nintendo originals, of course).
...
A massive downside to using SVG is these files are just murder on your computer. At times I find myself in an Inkscape loop where I do a single thing, wait 30 seconds for it to finish, do another thing, wait 30 seconds, repeat over and over... Sometimes these SVG files are really not fun to work with.

At the time I didn't know if this would make a difference, but I now think it does.
...
but it's still massive, about the size of a washing machine. It can print at up to 4800 DPI, in color or black and white. All in all it's an excellent printer and I think I could get it to print even better E-Reader cards than I did before, knowing what I know now.

#### Offset Printing Presses

Presses are a giant step up from printers and are more about printing at scale. They are possitively massive, requiring a dedicated room. Here is an offset printer at a print shop near me.
...
<wisdom>
	...
	small test run, I just got the sheets and cut the cards out myself. That is
	why the cards are kinda ratty looking on the edges.
</wisdom>

For this test run, I had the same dot strip used throughout all of the cards, but used my SVG dot strip tool to vary the shape, size, and gap of the dots. This test run was also done on three different types of card stock. The result was three SVG dot strip variations that scanned well. One in particular, circle with a 50% gap, scans the best.
...
I concluded that these cards scan dramatically better than the Solitaire cards ever did, and come the closest to Nintendo scan quality I have ever seen. Nintendo cards still scan better, but not by much.
...
In [part 1](https://mattgreer.dev/blog/printing-ereader-cards/) I recommended printing at 600 DPI. But then in [part 2](https://mattgreer.dev/blog/printing-ereader-cards-more-learnings/), I recommended 1200 DPI. But these Kodak cards were printed at 600 DPI, so ... what gives?
...
The Kodak press works at 600 DPI, and I found a 50% gap to be ideal. But if it printed at 1200 DPI, I might have found a 62.5% or 37.5% gap work even better. But like I said, these are the best scanning cards I've ever seen, so DPI is not the be all end all here.
...
- They are made entirely from a single black ink.
- There is a discernable gap between every dot.
- There is virtually no noise at all outside of the dots, just the pure white of the paper.
...
This card does scan in an E-Reader, but just barely. This is by far the worst scanning card that can still scan I've encountered. The printer is using color inks to help form the dots, and I am guessing the scanner is ignoring them, causing the scanner to be forced to work with very small dots.
...
Unfortunately the photo doesn't do this justice, but the Epson dots are both more reflective, and kind of washed out. Instead of a vibrant black, they are more of a dark grey in many places. Where as the Kodak dots are a nice solid black everywhere, and more matte to boot.
...
again each dot is very distinct. Where as with the Kodak card just above, the dots bleed into each other. How important is maintaining a gap between the dots? My guess is not that important.

It can't be ingored though. My test cards had gaps ranging from 12% to 75%. And for sure, the different sized dots impacted scanning. The cards with a 75% dot gap had pretty small dots on them, but they would scan. However, not very well. Cards with very little gap caused the dots to really bleed together, and those would not scan at all.
...
I'm not sure how much of a factor this plays in scanning. The Kodak cards scan _very_ close to as well as Nintendo's. My suspicion is if I had used an offset press instead, I'd probably have cards that scan exactly as we as Nintendo's did. So my guess is less noise is better, but some noise is not a deal breaker.

## Why not get cards offset printed?

I've never had cards printed on an offset press. All of my cards were digitally printed in one way or another. I would _really_ like to do an offset run and see how those cards compare. But there are two factors here
...
- The print shops won't let me :) I've asked numerous shops if I can get an offset run done and they have all shot me down.

I'm not too worried about this. The Kodak press's results are fantastic, and inexpensive. For now, I think digital presses are a perfectly fine compromise. I am hoping as I work with the printers more, I will eventually get that holy grail offset run done :)
