...
class Vehicle {
  ...
  calcedTurnDecisionDots: Array<{
    turn: number;
    turnedAngleIndex: number;
    magnitude: number;
    dot: number;
    best: boolean;
  }> = [];
  ...
  update(waypoints: Waypoint[]) {
    ...
    if (lastShouldDrift && !this.shouldDrift) {
      ...
      driftBoost =
        1 + Math.min(this.driftDuration / 60, 2) * Vehicle.DRIFT_BOOST_FACTOR;
  ...
  calcBoundingSpheres(): [Sphere, Sphere] {
    const offset = Vehicle.LENGTH / 4;
    const radius = Vehicle.LENGTH / 4;
  ...
  calcNearnessSphere(): Sphere {
    return {
      x: this.x,
      y: this.y,
      radius: Math.max(Vehicle.WIDTH, Vehicle.LENGTH) / 2,
    };
  }
  ...
  clone(): Vehicle {
    ...
    (Object.keys(this) as Array<keyof Vehicle>).forEach((key) => {
