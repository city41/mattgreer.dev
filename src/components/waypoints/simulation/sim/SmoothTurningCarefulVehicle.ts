...
class SmoothTurningCarefuleVehicle implements IVehicle {
	...
	calcVelocityAngleChangeRate(
		...
		const rate = (velocityAngleSpan / framesTillWp) * turnDecision;
	...
	handleAcceleration() {
		...
		const accelValue = this.shouldAccelerate ? this.accelValue : 0;
		const acceleration = accelValue - FRICTION - airDrag;
	...
	determineIfShouldAccelerate(targetWaypoint, nextWaypoint): boolean {
		...
		const distanceFromTargetToNext = getDistance(
			targetWaypoint.x,
			targetWaypoint.y,
			nextWaypoint.x,
			nextWaypoint.y
	...
	update(waypoints: Waypoint[]) {
		...
		// and move the car
		this.shouldAccelerate = this.determineIfShouldAccelerate(
			currentWaypoint,
			nextWaypoint
		);
	...
	clone(): IVehicle {
		const clone = new SmoothTurningCarefuleVehicle(
			this.x,
			this.y,
			this.accelValue,
			this.color
		...
		(Object.keys(this) as Array<keyof SmoothTurningCarefuleVehicle>).forEach(
			(key) => {
...
export { SmoothTurningCarefuleVehicle };
